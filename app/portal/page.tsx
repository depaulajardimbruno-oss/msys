...
export default async function PortalPage() {
  ...
  // Busca o treinamento mais recente do participante
  const { data: participante } = await supabase
    .from('participantes')
    .select('*, treinamento:treinamentos(*, cliente:clientes(nome))')
    .eq('profile_id', session.user.id)
    .order('created_at', { ascending: false })
    .limit(1)
    .single()

  const t = (participante as any)?.treinamento
  ...
  const { data: sessoes } = await supabase
    .from('sessoes').select('*').eq('treinamento_id', t.id).order('inicio')

  const { data: materiais } = await supabase
    .from('materiais').select('*').eq('treinamento_id', t.id)

  const jaAvaliou = await supabase
    .from('avaliacoes')
    .select('id')
  ...
  return (
    <div className="p-6 max-w-3xl">
      ...
      {/* CTA Avaliação */}
      {!jaAvaliou.data && t.status === 'concluido' && (
